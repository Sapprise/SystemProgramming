...
char *if_write_dir(char *read, char *write) {
    struct stat st;
    char *psrc, *pdes;
    int len = 0;
    pdes = write;
...
void cp_dir(char *read, char *write) {
    struct stat src_st;
    ...
    if (argc < 3) {
        ...
        for (int i = 1; i < argc - 1; i++) {
            ...
            if (S_ISDIR(src_st.st_mode)) {
                mkdir(if_write_dir(argv[i], argv[argc - 1]), 0755);
            } else {
                fwd = open(if_write_dir(argv[i], argv[argc - 1]), O_WRONLY | O_CREAT | O_TRUNC, src_st.st_mode);
                cp_date(frd, fwd);
            }
            printf("%s\n", if_write_dir(argv[i], argv[argc - 1]));
        }
    }
    return 0;
}
